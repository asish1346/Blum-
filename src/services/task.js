...
class TaskService {
  ...
  async verifyTask(user, task) {
    ...
    if (!user?.database?.tasks) {
      user.log.log(
        colors.yellow(
          `Task ${colors.blue(taskName)} doesn't have an answer yet, will retry later ID: ${(task.id)}`
        )
      );
      return;
    }
    ...
    if (!taskDatabase) {
      user.log.log(
        colors.yellow(
          `Task ${colors.blue(taskName)} doesn't have an answer yet, will retry later ID: ${colors.blue(task.id)}`
        )
      );
      return;
    }
